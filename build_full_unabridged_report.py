    ...
    def save(self):
        ...
        for idx, state in enumerate(self._saved_page_states):
            ...
            if 2 <= page_num <= 10:
                ...
                self.drawCentredString(306, 36, roman_str)
                self.drawString(54, 750, "Tribhuvan University — Faculty of Humanities & Social Sciences")
                ...
                self.drawRightString(558, 36, str(chap_page))
                self.drawString(54, 36, "FinSight – Smart Personal Finance & Budget Tracker")
                self.drawString(54, 750, "BCA Project Report — Department of Computer Applications")
                self.setStrokeColor(colors.HexColor("#CBD5E1"))
                self.setLineWidth(0.5)
                self.line(54, 742, 558, 742)
                self.line(54, 48, 558, 48)
...
def build_pdf():
    ...
    story.append(Paragraph("1. To create a specialized platform for individuals where they can log income and expense transactions, set monthly budgets, define savings goals, and receive intelligent alerts with", st['bullet']))
    story.append(PageBreak())
    ...
    # ---------------------------------------------------------
    story.append(Spacer(1, 10))
    story.append(Paragraph("complete control over their own financial data.", st['body']))
    ...
    story.append(Spacer(1, 4))
    story.append(Image("/tmp/finsight_diagrams/fig_1_1.png", width=440, height=120))
    ...
    story.append(Spacer(1, 4))
    story.append(Image("/tmp/finsight_diagrams/fig_3_1.png", width=380, height=180))
    ...
    story.append(Spacer(1, 4))
    story.append(Image("/tmp/finsight_diagrams/fig_3_2.png", width=420, height=105))
    ...
    story.append(Spacer(1, 4))
    story.append(Image("/tmp/finsight_diagrams/fig_3_3.png", width=420, height=130))
    ...
    story.append(Image("/tmp/finsight_diagrams/fig_3_4_5.png", width=440, height=80))
    ...
    story.append(Image("/tmp/finsight_diagrams/fig_3_6.png", width=420, height=110))
    story.append(Paragraph("Figure 3.6: Sequence Diagram for User Login and Transaction Creation", st['caption']))

    story.append(Spacer(1, 4))
    story.append(Image("/tmp/finsight_diagrams/fig_3_7.png", width=420, height=85))
    ...
    story.append(Spacer(1, 4))
    story.append(Image("/tmp/finsight_diagrams/fig_3_8_9.png", width=440, height=120))
    ...
    story.append(Paragraph(
        ...
        "Epsilon Floor is applied: σ_floored = max(σ, |µ| * 0.05). Thresholds of |z| >= 2.0 (low/moderate) and |z| >= 3.0 (high) are used for severity. "
        "A minimum sample size threshold (MIN_SAMPLES = 5) prevents false alarms on new categories.", st['body']
    ))
    story.append(Image("/tmp/finsight_diagrams/fig_3_10.png", width=440, height=85))
    ...
    story.append(Image("/tmp/finsight_diagrams/fig_3_11.png", width=440, height=80))
    ...
    story.append(Image("/tmp/finsight_diagrams/fig_3_12.png", width=440, height=80))
